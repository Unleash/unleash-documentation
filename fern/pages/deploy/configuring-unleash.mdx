...
PostgreSQL is the only supported database; MySQL and MariaDB are not supported. Managed PostgreSQL services, such as AWS RDS, Azure Database for PostgreSQL, or GCP Cloud SQL, are also supported. For the minimum PostgreSQL version required by each Unleash version, see [Upgrading Unleash](/deploy/upgrading-unleash).
...
| `PROMETHEUS_API`                                  | none          | The base URL of a Prometheus-compatible query API. Used to populate the [Network view](/concepts/network-view#configure-a-data-source) dashboards. |
