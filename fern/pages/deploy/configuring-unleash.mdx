...
description: "Learn how to configure self-hosted Unleash using Docker environment variables for database, URLs, email, auth, and security."
max-toc-depth: 3
og:title: Configure Unleash
...
PostgreSQL is the supported database. This includes managed PostgreSQL services, such as AWS RDS, Azure Database for PostgreSQL, and GCP Cloud SQL. MySQL and MariaDB are not officially supported.

Each Unleash major version has a minimum supported PostgreSQL version:

| Unleash version | Minimum PostgreSQL version |
|-----------------|----------------------------|
| 8.x             | 15                         |
| 7.x             | 14                         |
| 6.x             | 13                         |

For details on upgrading between versions, see [Upgrading Unleash](/deploy/upgrading-unleash).
...
| Environment variable | Default value                  | Description                                                                        |
| :------------------- | :----------------------------- | :--------------------------------------------------------------------------------- |
| `UNLEASH_URL`        | `http://localhost:4242`        | Public URL where your Unleash instance can be accessed.                            |

## Configure the Unleash Enterprise Edge public URL

Use the `EDGE_URL` environment variable to set the public URL for your [Enterprise Edge](/unleash-edge) instance. For example, `https://yourcompany.edge.getunleash.io`. This URL appears on the **Admin > API access** page alongside the Client and Frontend API URLs, so your team can copy it when connecting SDKs to Edge.

| Environment variable | Default value                  | Description                                                                        |
| :------------------- | :----------------------------- | :--------------------------------------------------------------------------------- |
| `EDGE_URL`           | N/A                            | Public URL where your Unleash Enterprise Edge can be accessed.                     |
...
| `PROMETHEUS_API`                                  | N/A           | The base URL of a Prometheus-compatible query API. Used to populate the [Network view](/concepts/network-view#configure-a-data-source) dashboards. |

Unleash exposes performance metrics in Prometheus format at `/internal-backstage/prometheus`. For a sample Prometheus scrape configuration, see [Network view](/concepts/network-view#configure-a-data-source).
