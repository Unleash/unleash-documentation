...
description: "Upgrade self-hosted Unleash safely: review major version changes, Docker guidance, backups, PostgreSQL, Node.js, and API updates."
max-toc-depth: 2
og:title: Upgrade Unleash
...
Unleash v8, released in June 2026, raises the minimum tested PostgreSQL version, includes an open-source license change, and removes several deprecated schema properties. Ensure you address the following before or during your upgrade to v8.

### Database requirement: PostgreSQL v15+

Unleash v8 raises the minimum tested PostgreSQL version to **15**. Unleash v8 does not currently use any PostgreSQL 15-only features, so existing instances on PostgreSQL 14 may continue to work. However, only PostgreSQL 15 and later are officially tested and supported. Plan to upgrade your PostgreSQL instance before or alongside your Unleash upgrade.

### Open-source license change to AGPLv3

Starting with v8.0.0, the source code in the [primary Unleash GitHub repository](https://github.com/Unleash/unleash) and the [`unleash-server` npm package](https://www.npmjs.com/package/unleash-server) move from the Apache 2.0 license to the GNU Affero General Public License v3 (AGPLv3). Official open-source Docker images remain under Apache 2.0, so running a pre-built Unleash container without modifying the source is unaffected. Unleash SDKs, Unleash Enterprise, and Unleash Cloud are unaffected.

For the full reasoning and a use-case breakdown, see [Evolving our open source commitment](https://www.getunleash.io/blog/unleash-moving-to-agplv3).

### API changes

No API endpoints have been removed in v8. Several previously deprecated schema properties have been removed. If your integrations rely on these properties, update them before upgrading.

<Accordion title="Removed schema properties">

- **`lastSeenAt`**

  Removed from `featureSchema`. Use the per-environment `lastSeenAt` under each `environments` entry instead.
  Affected endpoints: `/api/admin/projects/{projectId}/features`, `/api/admin/projects/{projectId}/features/{featureName}`, `/api/admin/projects/{projectId}/features/{featureName}/clone`, `/api/admin/projects/{projectId}/features/{featureName}/environments/{environment}/on`, `/api/admin/projects/{projectId}/features/{featureName}/environments/{environment}/off`, `/api/admin/projects/{projectId}/health-report`, `/api/admin/search/features`, `/api/admin/user/profile`, `/api/admin/features-batch/export`, `/api/admin/features-batch/import`, and `/api/admin/features-batch/validate`.

  ---

- **`strategies`**

  Removed from `featureSchema`. Use the per-environment `strategies` under each `environments` entry instead.
  Affected endpoints: same as `lastSeenAt`.

  ---

- **`variants`**

  Removed from `featureSchema`. Use the per-environment `variants` under each `environments` entry instead.
  Affected endpoints: same as `lastSeenAt`.
...
- **`type` and `value`**
  ...
  Removed from the feature tag item shape on `featureSchema` in batch endpoints.
  Affected endpoints: `/api/admin/features-batch/export`, `/api/admin/features-batch/import`, and `/api/admin/features-batch/validate`.

  ---

- **`featureLinkSchema`**

  The schema has been removed entirely.
...
<Accordion title="Removed API endpoints">
...
<Accordion title="Previously deprecated endpoints restored in v7">
...
<iframe
  width="100%"
  height="500"
  src="https://www.youtube-nocookie.com/embed/qmusq_9mE2E"
  title="Upgrading from v3 to v5"
  frameborder="0"
...
Read more in the [API token documentation](/concepts/api-tokens-and-client-keys).
