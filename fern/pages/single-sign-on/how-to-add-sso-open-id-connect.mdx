...
![admin-authentication](/assets/sso-oidc.png)
</Frame>
...
| OIDC_ENABLE_SINGLE_SIGN_OUT     | Should Unleash call the single signout of the OIDC endpoint (defaults to false)
| OIDC_EXTRA_SCOPES               | Space-separated list of additional scopes to request during login, beyond the default `openid email profile` and `groups` if group syncing is enabled.                           | No       | custom1 custom2     |
