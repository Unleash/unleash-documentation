...
description: Configure the Unleash MCP server to automate feature flag management in Kiro IDE and CLI.
keywords: kiro, mcp, model context protocol, ai, llm, coding assistants, feature flags, automation
max-toc-depth: 3
slug: integrate/kiro
---

The Unleash MCP server connects Kiro to your Unleash instance, enabling AI-assisted feature flag management.
...
- **Kiro CLI** — Terminal-based workflows and batch operations (successor to Amazon Q Developer CLI)
...
<Tab title="Command Palette">
    ...
    Save the file. Open the **MCP Servers** tab in the Kiro panel to confirm the Unleash server appears with its tools listed.

    You can also check MCP logs via the **Output** tab by selecting "Kiro - MCP Logs" from the dropdown.
    </Step>
...
<Tab title="Create file manually">
    ...
    The file contains no credentials, only variable references that expand at runtime. You can safely commit it to version control.
    </Step>
...
**`autoApprove` and `disabledTools`**

Kiro supports two unique MCP properties:
...
The Kiro CLI is the successor to the Amazon Q Developer CLI, which was deprecated in November 2025.
...
The CLI's pipe-friendly interface enables batch operations. Combine standard Unix tools with the Kiro CLI to process data that the MCP server cannot access directly.

```bash
# Evaluate a diff for flag requirements
git diff main...feature-branch | kiro-cli -p "Analyze this diff. Use evaluate_change to determine which changes need feature flags."

# Generate a flag audit report
kiro-cli -p "List all feature flags and identify any at 100% rollout for more than 14 days. Use get_flag_state for each and generate a cleanup report." > audit.md

# Find commits that should have been flagged
git log --oneline -20 | kiro-cli -p "Review these commits. For each, use evaluate_change to determine if it should have been feature flagged. Report gaps."
```

<Note>
The Kiro CLI shares credit limits with the IDE.
</Note>
...
Kiro provides enterprise controls through AWS IAM Identity Center and Okta/Microsoft Entra sign-in.
...
- Check MCP logs via **Output** tab → "Kiro - MCP Logs"
</Accordion>
...
- [Spec-Driven Feature Flags: How Kiro and Unleash Turn Requirements into Safe Releases](/blog/kiro-unleash-spec-driven-feature-flags)
