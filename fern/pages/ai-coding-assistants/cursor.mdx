...
Cursor supports two approaches for credential management: `envFile` (simpler) and environment variable interpolation (more flexible). Choose one.

<Tabs>

<Tab title="envFile (recommended)">
...
<Tab title="Environment variable interpolation">
...
<Warning>
The one-click flow writes credentials directly into `.cursor/mcp.json`. This is fine for personal use, but for team projects you should replace the inline values with an `envFile` reference or `${env:NAME}` variables so the configuration file is safe to commit. See the "Create file manually" tab for details.
</Warning>
...
Create `.cursor/mcp.json` at your project root. The configuration depends on which credential approach you chose above.
...
<Card title="Prevent duplication" icon="copy">
