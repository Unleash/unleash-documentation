...
The Unleash MCP server runs as a local stdio process. Configuration uses the same npm package and credentials across all OpenCode interfaces.

First, create a credentials file and source it from your shell profile:
...
The CLI shares the same `opencode.json` configuration as the TUI. If you already configured the MCP server for the TUI, the CLI picks it up automatically.

If you haven't configured it yet, create the `opencode.json` file as shown in the OpenCode TUI tab.

**Non-interactive execution with `opencode run`:**

```bash
opencode run "Check if any feature flags should be cleaned up"
```

**Headless server mode with `opencode serve`:**

```bash
# Start server (keeps MCP connections alive)
opencode serve &

# Run multiple evaluations against the same server
