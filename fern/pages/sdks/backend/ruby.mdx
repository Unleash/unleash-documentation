...
og:title: Ruby SDK
description: Set up the Unleash Ruby SDK to evaluate feature flags with context, variants, bootstrap, custom strategies, and Rails support.
keywords: [unleash, ruby, sdk, feature flags, feature toggles]
max-toc-depth: 3
last-updated: May 11, 2026
---

<Markdown src="/snippets/sdks/backend-intro.mdx" language="Ruby" />
...
| `environment`         | Unleash context option, for example, `prod` or `dev`. Not yet in use. **Not** the same as the SDK's [Unleash environment](/concepts/environments).                                                                                                                                         | N         | String                            | `default`                                      |
...
When using [Puma](https://github.com/puma/puma) with multiple workers configured in `puma.rb`:
...
When using [Phusion Passenger](https://github.com/phusion/passenger), the Unleash client needs to be configured and instantiated inside the `PhusionPassenger.on_event(:starting_worker_process)` code block due to [smart spawning](https://www.phusionpassenger.com/library/indepth/ruby/spawn_methods/#smart-spawning-caveats):
...
When using [Sidekiq](https://github.com/sidekiq/sidekiq), the code block for `Unleash.configure` must be set beforehand.
...
For the full method signatures, see [client.rb](https://github.com/Unleash/unleash-ruby-sdk/blob/main/lib/unleash/client.rb).
...
You can add [custom activation strategies](/concepts/custom-activation-strategies) using configuration.
...
If you use [custom strategies](#custom-strategies) or override built-in ones, read the complete [v6 migration guide](https://github.com/Unleash/unleash-ruby-sdk/blob/main/./v6_MIGRATION_GUIDE.md) before upgrading.
