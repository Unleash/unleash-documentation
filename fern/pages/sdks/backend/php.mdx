...
If you're migrating from 1.x to 2.x, you can read the [migration guide](https://github.com/Unleash/unleash-php-sdk/blob/main/doc/migrating-1.x-2.x.md).
...
See [event documentation here](https://github.com/Unleash/unleash-php-sdk/blob/main/doc/events.md).
...
To use the Proxy SDK, you need to call `withProxy($apiKey)` on the builder. The `$apiKey` needs to be a [frontend token](/concepts/api-tokens-and-client-keys#front-end-tokens). Note that `withProxy($apiKey)` is in lieu of setting the API key header.
...
As of version 1.12, the Proxy SDK requires [Edge](/unleash-edge), so the `appUrl` needs to point to the Edge server.
