...
To use Frontend API mode, call `withProxy($apiKey)` on the builder. The `$apiKey` needs to be a [frontend token](/concepts/api-tokens-and-client-keys#frontend-tokens). Note that `withProxy($apiKey)` sets the API key directly, so you don't need to set the API key header separately.
