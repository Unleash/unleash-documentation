...
og:title: PHP SDK
description: PHP SDK for integrating with Unleash feature management platform
keywords: [unleash, php, sdk, feature flags, feature toggles]
max-toc-depth: 3
---

<Markdown src="/snippets/sdks/backend-intro.mdx" language="PHP" />

## Requirements

- PHP 7.2 or newer
- A [PSR-18](https://packagist.org/providers/psr/http-client-implementation) HTTP client (e.g. [Guzzle](https://packagist.org/packages/guzzlehttp/guzzle))
- A [PSR-17](https://packagist.org/providers/psr/http-factory-implementation) request factory (e.g. [nyholm/psr7](https://packagist.org/packages/nyholm/psr7))
- A [PSR-16](https://packagist.org/providers/psr/simple-cache-implementation) cache (e.g. [Symfony Cache](https://packagist.org/packages/symfony/cache))

For event support, you also need `symfony/event-dispatcher`. See the [event documentation](https://github.com/Unleash/unleash-php-sdk/blob/main/doc/events.md).
...
By default the SDK uses the backend endpoints on the Unleash API. You can also use Frontend API mode, which is a lightweight mode that uses the Frontend API endpoints. Frontend API mode provides a substantial performance improvement when using a large set of feature toggles (10K+).

To use Frontend API mode, call `withProxy($apiKey)` on the builder. The `$apiKey` needs to be a [frontend token](/concepts/api-tokens-and-client-keys#front-end-tokens). Note that `withProxy($apiKey)` sets the API key directly, so you don't need to set the API key header separately.
...
As of version 1.12, Frontend API mode requires [Edge](/unleash-edge), so the `appUrl` needs to point to the Edge server.
...
when this happens, for more information see [events documentation](https://github.com/Unleash/unleash-php-sdk/blob/main/doc/events.md).
</Note>
...
[More here](/concepts/activation-strategies).
...
// even though the client will not attempt to register, you can still use isEnabled()
...
## Custom headers with middleware
...
Check out our guide for more information on how to build and scale [feature flag systems](/guides/feature-flag-best-practices).

## Symfony integration

If you're using Symfony, the [Unleash Symfony Bundle](https://github.com/Unleash/unleash-client-symfony) provides framework-level integration with automatic dependency injection and configuration.

## Migrating to v2

If you're upgrading from v1, read the [v2 migration guide](https://github.com/Unleash/unleash-php-sdk/blob/main/doc/migrating-1.x-2.x.md).
