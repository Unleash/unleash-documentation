...
You can also pass an [Unleash context](/concepts/unleash-context) to evaluate flags for a specific user. Refer to the [Unleash context](#unleash-context) section for details.
...
By default, bootstrap data overrides any locally cached data. Set `ToggleOverride` to `false` to keep cached data when available.
