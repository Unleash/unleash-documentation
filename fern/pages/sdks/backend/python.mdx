...
og:title: Python SDK
description: Set up the Unleash Python SDK to evaluate feature flags with context, variants, fallbacks, bootstrap, events, and custom cache.
...
The Python SDK lets you tap into its behavior through [impression data](/concepts/impression-data) and lifecycle events. Provide an `event_callback` function when you initialize the client.

<Note>
The SDK does not include a built-in event bus — you'll need to provide your own. The examples below use [Blinker](https://pypi.org/project/blinker/) to send signals.
</Note>

The callback receives a single `UnleashEvent`. The SDK emits the following event types:
- **Impression events**: emitted when `is_enabled()` or `get_variant()` is called for a flag with [impression data](/concepts/impression-data) enabled.
