...
The SDK automatically attaches `appName`, `environment`, and `origin` labels to all impact metrics.

Impact metrics are available on the client instance returned by `NewClient(...)`. The global client created with `Initialize(...)` does not expose impact metrics.

Publish metrics through `client.ImpactMetrics()`:
...
func main() {
	...
	metrics := client.ImpactMetrics()

	metrics.DefineCounter("request_count", "Total number of HTTP requests")
	metrics.IncrementCounter("request_count")

	metrics.DefineGauge("active_users", "Current number of active users")
	metrics.UpdateGauge("active_users", 42)
	...
	metrics.ObserveHistogram("request_time_ms", 125)
}
```
...
Use histograms to measure the distribution of values, such as request duration or response size. Unleash automatically calculates percentiles (p50, p95, p99). You can pass custom bucket boundaries as additional arguments to `DefineHistogram`. If omitted, the SDK uses default buckets.
