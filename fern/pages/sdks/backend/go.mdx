...
Impact metrics are application-level time-series metrics stored and visualized in Unleash. Use them to track outcomes such as request volume, error rates, and latency for feature rollouts.

The SDK automatically adds `appName` and `environment` labels to all impact metrics.

Use `NewClient(...)` and publish metrics through `client.ImpactMetrics()`:
...
```go
metrics.DefineHistogram("request_time_ms", "Request processing time in milliseconds", 50, 100, 200, 500, 1000)
metrics.ObserveHistogram("request_time_ms", 125)
```

Impact metrics are batched and sent on the same schedule as standard SDK metrics (`WithMetricsInterval`).
