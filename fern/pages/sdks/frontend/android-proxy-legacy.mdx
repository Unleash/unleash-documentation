...
description: Legacy Android SDK for feature flags - now deprecated in favor of the modern Android SDK
og:site_name: Unleash
og:title: Android SDK (Legacy)
keywords: Android SDK legacy, feature flags, mobile development, deprecated SDK
max-toc-depth: 3
---
...
To connect this SDK to your Unleash instance's [front-end API](/api#frontend-api), use the URL to your Unleash instance's front-end API (`<unleash-url>/api/frontend`) as the `proxyUrl` argument. For the `clientKey` parameter, use a `FRONTEND` token generated from your Unleash instance. Refer to the [API tokens documentation](/concepts/api-tokens-and-client-keys) for more information.
...
In [the sample app](https://github.com/Unleash/unleash-android-proxy-sdk/blob/main/./samples/android/app/src/main/java/com/example/unleash/MainActivity.kt)
