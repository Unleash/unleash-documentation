---
title: Vue SDK
description: Integrate feature flags in your Vue.js applications using the Unleash Vue SDK
og:site_name: Unleash
og:title: Vue SDK
keywords: Vue SDK, Vue.js SDK, feature flags, web development, frontend SDK
max-toc-depth: 3
---

This Frontend Vue SDK is designed to help you integrate with Unleash and evaluate feature flags inside your application.
...
This library is meant to be used with [Unleash Edge](https://github.com/Unleash/unleash-edge/) or the [Unleash front-end API](/api#frontend-api).
...
The [Developer Toolbar](/integrate/toolbar) provides runtime flag overrides during development.
...
export function useUnleash() {
  ...
  onMounted(async () => {
    const client = new UnleashClient({ /* your config */ })

    if (import.meta.env.DEV) {
      unleashClient.value = initUnleashToolbar(client, { themePreset: 'dark' })
    } else {
  ...
  return { unleashClient, isReady }
}
```

Refer to the [complete Vue example on GitHub](https://github.com/Unleash/toolbar/tree/main/examples/vue) for a full implementation.
