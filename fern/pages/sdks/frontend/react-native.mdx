...
<ParamField path="url" type="string" required={true}>
  The URL of your [Unleash Edge](/unleash-edge) instance or [Frontend API](/api#frontend-api). For the Frontend API, use `https://<your-unleash-instance>/api/frontend`.
</ParamField>
...
<ParamField path="context" type="object" default="{}">
  The initial [Unleash context](/concepts/unleash-context).
</ParamField>
...
<ParamField path="impressionDataAll" type="boolean" default="false">
  If `true`, the SDK emits impression events for all `isEnabled` and `getVariant` calls, including flags that are disabled.
</ParamField>

<ParamField path="environment" type="string" default="default">
  Sets the `environment` field in the Unleash context. This does not affect which Unleash environment the SDK connects to.
</ParamField>
...
To connect to Unleash using the [Frontend API](/api#frontend-api), set `url` to `<your-unleash-instance>/api/frontend`. To connect through [Unleash Edge](/unleash-edge), set `url` to your Edge instance URL.

You must use the frontend token as the `clientKey`, see [API tokens](/concepts/api-tokens-and-client-keys) for how to generate a token.

### Check if the SDK is ready

When `FlagProvider` mounts, the SDK fetches flag configuration from Unleash. Until that first fetch completes, all hooks return default values (`false` for `useFlag`, `{ name: "disabled", enabled: false }` for `useVariant`).

Use the `useFlagsStatus` hook to check whether the SDK has loaded flag data:
...
`flagsReady` requires a successful network request before it becomes `true`, even if cached data exists from a previous session. This means there is a brief loading state on every app launch when you gate rendering on `flagsReady`.
...
| Event | Description |
|---|---|
| `ready` | The SDK completed its first successful flag fetch. |
...
If you need to capture events that fire during initialization (such as `ready`), use the [deferred client start](#defer-client-start) pattern to attach listeners before the client starts.
...
const testConfig = {
  url: 'http://localhost', // not used during tests
...
- Verify that `url` points to your Unleash Edge instance or Frontend API endpoint (`<your-unleash-instance>/api/frontend`). The URL must include the full path.
...
- Confirm that the flag is enabled in the correct Unleash environment and that your frontend token has access to that environment.
