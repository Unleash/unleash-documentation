...
The SDK stores flag configuration in `localStorage` and synchronizes with Unleash or [Unleash Edge](/unleash-edge) in the background. Because flags are stored in the browser, the SDK can bootstrap itself the next time the user visits the same page.
