...
OpenFeature can be useful when you want a common feature flag interface across services, frameworks, or vendors. The trade-off is that the OpenFeature API models only the capabilities that feature flag systems have in common. It does not cover Unleash-specific capabilities, such as the full [Unleash context](/concepts/unleash-context) for fine-grained targeting, [impression data](/concepts/impression-data), [impact metrics](/concepts/impact-metrics), and automated [release management](/concepts/release-management-overview) with milestones. To get the most out of Unleash, we recommend that you use the Unleash SDKs directly.
...
Unleash also helps maintain the following ecosystem-owned OpenFeature providers, keeping their Unleash integrations aligned with Unleash behavior:
