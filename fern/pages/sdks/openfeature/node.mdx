...
`setProviderAndWait` resolves once the Unleash client is ready and synchronized. It rejects on the first Unleash error instead of waiting forever, so it cannot hang on a misconfigured connection. The Unleash client keeps retrying in the background, and the provider recovers once a fetch succeeds.
...
The provider adds all other fields to the Unleash context [properties](/concepts/unleash-context#the-properties-field). Strings and numbers pass through unchanged, while booleans and dates are converted to strings. It discards nested values, such as arrays and objects.
