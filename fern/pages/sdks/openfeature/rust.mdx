...
Install the provider together with the OpenFeature Rust SDK and the Unleash Rust SDK. Your application imports `ClientBuilder` from the `unleash-api-client` crate to build the provider configuration.

```bash
cargo add unleash-openfeature-rust-provider open-feature unleash-api-client
```

To use async-std instead of Tokio, disable the default features:
