...
Create an `Unleash::OpenFeature::Provider::UnleashFlagProvider` with the same options you would pass to the Unleash Ruby SDK client, then register that provider with OpenFeature.
...
Use a backend client token with the Ruby provider. The provider wraps the backend Ruby SDK, evaluates flags locally.
...
Shut down the provider when your application exits.
