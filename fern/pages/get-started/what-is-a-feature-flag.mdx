...
- Release, experiment, kill switch, operational, permission, and sunset flags each address a different stage or need in the feature lifecycle.
...
<Tab title="Sunset flags">
<Button small outlined className="version-button" href="/support/availability">v8.0</Button>

- **Purpose**: Coordinate the planned removal of an existing feature.
- **Benefits**: A controlled, observable path to retirement that reduces the risk of breaking dependent users or systems.
- **Lifespan**: Short-lived; typically a few weeks to a few months, ending when the feature is fully removed.

Sunset flags wrap a feature that you plan to retire, letting you gradually disable it for selected user segments while you monitor the impact. You can pause the rollout or re-enable the feature if downstream issues surface, then remove the code path once the feature is fully decommissioned.

Sunset flags are inherently temporary. Their lifespan should align with the deprecation timeline you communicate to users and dependent teams. Once the feature is removed from the codebase, archive the flag to avoid leaving stale references behind.
</Tab>
</Tabs>
...
<Accordion title="Trunk-based development">
...
<Accordion title="Feature management">
...
<Accordion title="Compliance with FedRAMP, SOC 2, ISO27001, Air-gap, and others">
...
A common practice is automating flag clean-up using coding assistants or the [Unleash MCP server](/integrate/mcp).
</Accordion>
