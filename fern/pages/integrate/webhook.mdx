...
og:title: Webhook
description: "Set up Unleash webhooks to send feature flag, project, environment, and change events to external tools with headers and retries."
max-toc-depth: 2
...
**Body template** Used to override the body template used by Unleash when performing the HTTP POST. You may format your message using a [Mustache template](https://mustache.github.io). You will have the Unleash event format available in the rendering context:

<EndpointResponseSnippet endpoint="GET /api/admin/events" />
