...
- **Screen reader support**: Dialog, tab, and radio group semantics, labeled controls, and visible focus indicators
...
| `shortcut` | `string` \| `false` | `'mod+shift+f'` | Keyboard shortcut that toggles the panel. `mod` is Cmd on macOS and Ctrl elsewhere. Set to `false` to register no global key listener. |
| `focusOnOpen` | `'panel'` \| `'search'` \| `'context'` | `'panel'` | Where to place focus when the panel opens. |
| `trapFocus` | `boolean` | `true` | Keep `Tab` and `Shift + Tab` cycling inside the panel while it's open. |
...
- **Minimize it**: Click the minimize (`_`) button in the panel header, or press `Esc` while focus is inside the panel, to collapse the panel back to the floating icon.
...
To toggle the toolbar panel open and closed:
...
| Key | Action |
|-----|--------|
| `Esc` | Minimize to the floating icon |
| `Tab` / `Shift + Tab` | Move between controls; cycles within the panel |
...
While the panel is open, `Tab` cycles within it instead of moving to the page behind. This is a soft trap: the page is never marked `inert` and the panel doesn't claim `aria-modal`, so mouse interaction with your app is unaffected. `Esc` releases the trap and returns focus to the floating icon, or to whatever held focus before if the icon isn't rendered. Set `trapFocus: false` to disable this.
...
- The panel is a `dialog` labeled by its title.
