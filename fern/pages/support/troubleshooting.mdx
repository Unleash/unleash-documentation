...
description: Guide to troubleshooting common Unleash feature flag issues including flags not being returned, users not being exposed, A/B test issues, and CORS errors.
keywords: ["troubleshooting", "feature flags", "debugging", "CORS", "API tokens", "401", "403", "authentication errors", "stickiness", "variants", "A/B testing"]
...
-   Next, configure [strategy variants](/concepts/strategy-variants) to represent "enabled" and "disabled" states. You can assign percentages to these variants (e.g., 50% "enabled", 50% "disabled").
    ...
    ![Using enabled and disabled variants](/assets/enabled-disabled-variants.png)
    </Frame>
...
-   This approach can also be combined with more complex constraint-based targeting.
    ...
    <Frame background="subtle">
    ![Using enabled and disabled variants with constraints](/assets/enabled-disabled-variants-complex.png)
    </Frame>
...
## I get a 401 or 403 error from the API

A `401 Unauthorized` response means the request could not be authenticated. A `403 Forbidden` response usually means the token was recognized but is not allowed to do what it asked; some APIs also return `403` for invalid or wrong-type tokens. Work through the following checks to identify the cause.

#### Check the Authorization header

Pass the API token directly in the `Authorization` header, without a `Bearer` prefix:

```bash
curl -H "Authorization: <your-token>" https://<your-unleash-url>/api/client/features
```

A missing header, or a header your proxy strips or rewrites, results in a `401` error.

#### Match the token type to the API

Each API accepts one token type, and a valid token of the wrong type is rejected:

| API | Endpoint prefix | Required token type |
|-----|-----------------|---------------------|
| Client API | `/api/client` | [Backend token](/concepts/api-tokens-and-client-keys#backend-tokens) |
| Frontend API | `/api/frontend` | [Frontend token](/concepts/api-tokens-and-client-keys#frontend-tokens) |
| Admin API | `/api/admin` | [Personal access token](/concepts/api-tokens-and-client-keys#personal-access-tokens) or [service account token](/concepts/api-tokens-and-client-keys#service-account-tokens) |

The Frontend API returns the same `403` response whether the token is malformed, deleted, expired, or of the wrong type, so the status code alone does not tell you which of these applies. Verify each in turn: the token is a frontend token, it still exists in **Admin settings > Access control > API access**, and its project and environment scope covers the flags you are requesting.

#### Check the token scope and expiry

A token only accesses the projects and environment it was created for, and this scope is immutable; see [Check token access](#check-token-access) for how to read the scope from the token format. Requests with an expired token are rejected; see [token expiry and rotation](/concepts/api-tokens-and-client-keys#token-expiry-and-rotation).

#### Check for change requests on write operations

If Admin API write operations return `403` in a specific environment, check whether that environment requires [change requests](/concepts/change-requests). Environments with change requests enabled reject direct configuration changes; submit the change as a change request instead.

#### Check Unleash Edge token configuration

When connecting through [Unleash Edge](/unleash-edge) in edge mode, standard tokens are validated against the upstream Unleash instance and must be within the scopes Edge is configured to serve. If you use [pre-trusted tokens](/unleash-edge/deploy#pre-trusted-tokens), confirm that the token is explicitly configured and covered by a standard backend token for the same environment. In [offline mode](/unleash-edge/configure#offline-mode), confirm that the token is included in the client or frontend tokens configured at startup; these tokens do not need to exist in Unleash.
...
-   For a simple 50-50 A/B test, your variants should be configured accordingly (e.g., two variants, "A" and "B", with appropriate weighting or rollout distribution if not handled by a parent strategy).
    ...
    <Frame background="subtle">
    ![An example of variants configured for an A/B test](/assets/troubleshooting-flag-abn-test-unexpected-result-variants.png)
    </Frame>
...
-   To allow a specific origin:
    ```bash
    unleash-edge edge --cors-origin "https://your-application.com"
    ```
...
```bash
curl -s -I -X OPTIONS "https://<host>/<endpoint>" \
  -H "Origin: https://your-app.com" \
  -H "Access-Control-Request-Method: GET" \
  -H "Access-Control-Request-Headers: Authorization"
```

Check the response for the `Access-Control-Allow-Origin` header. If it is missing or doesn't match your application's origin, revisit your CORS configuration. Also check that the `Access-Control-Allow-Headers` header includes `authorization`; without it, the browser's preflight fails even when the origin is allowed.
...
1. In the Unleash Admin UI, go to **Admin settings > User config > Groups** and create a new group, for example `Unleash Admins`.
