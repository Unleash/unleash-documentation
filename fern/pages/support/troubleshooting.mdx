...
A `401 Unauthorized` response means Unleash could not authenticate the request; a `403 Forbidden` response means the token was recognized but is not allowed to do what it asked. Work through the following checks.
...
#### Check Unleash Edge token scopes

When connecting through [Unleash Edge](/unleash-edge), Edge validates tokens against its upstream Unleash instance. A token that does not exist upstream, or that is outside the scopes your Edge instance is configured to serve, is rejected even if it looks well-formed.
...
Check the response for the `Access-Control-Allow-Origin` header. If it is missing or doesn't match your application's origin, revisit your CORS configuration.
