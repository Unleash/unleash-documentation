...
<Steps>
  <Step title="Record your current runtime details">
  ...
  <Step title="Inventory your tokens">
  ...
  <Step title="Review the release notes">
    Review the [release notes](https://github.com/Unleash/unleash-edge/releases) for your target version.
  </Step>
...
<Steps>
  <Step>
    Start Edge with explicit, valid [backend tokens](/concepts/api-tokens-and-client-keys#backend-tokens) in `TOKENS`.
  </Step>
  <Step>
...
<Steps>
  ...
  <Step>
    Use `PRETRUSTED_TOKENS` only when you need temporary compatibility with legacy frontend token secrets during a [Proxy migration](/unleash-edge/migrate-from-proxy).
  </Step>
  <Step>
...
<Steps>
  <Step>
    Replace any service account or personal access tokens in `TOKENS` with a [backend token](/concepts/api-tokens-and-client-keys#backend-tokens) that covers the required projects and environments.
  </Step>
  <Step>
