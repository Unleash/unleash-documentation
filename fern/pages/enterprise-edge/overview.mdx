...
slug: /unleash-edge
description: Unleash Enterprise Edge is a lightweight caching layer designed to improve scalability, performance, and resilience for your feature flag infrastructure.
keywords: enterprise edge, edge, caching, scalability, performance, resilience, streaming, frontend sdks, backend sdks
og:site_name: Unleash
og:title: Enterprise Edge Overview
max-toc-depth: 3
---
...
<Frame background="subtle">
  ![Edge observability dashboard](/assets/unleash-enterprise-edge.png)
</Frame>
...
By default, Edge keeps its cache in sync with Unleash by polling the upstream API at a fixed interval. With streaming enabled, Edge opens a persistent connection to Unleash and receives feature flag changes as they happen. This reduces replication lag between Unleash and Edge from the polling interval down to milliseconds.

When streaming is enabled, Edge bootstraps its cache from the upstream `GET /api/client/delta` endpoint and then subscribes to `GET /api/client/streaming` for live updates. If the connection drops, Edge reconnects and resumes from the last event it received. While the connection is recovering, Edge continues to serve its last known state to connected SDKs, so SDK traffic is not interrupted.
...
Streaming is available on the Enterprise Edge build with an Unleash Enterprise instance. For self-hosted Unleash, your license must include Edge streaming. For setup, monitoring, and troubleshooting, see [Configure streaming](/unleash-edge/configure-streaming).
...
<Button small outlined className="version-button" href="/support/availability">v7.3</Button>
