...
Permissions to manage [release templates](/concepts/release-templates).
...
<Button small outlined className="version-button" href="/support/availability">v8.0</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>

Read-only access is available as an optional add-on. A read-only user holds the **Viewer** root role and can view all projects with the open or protected [collaboration mode](/concepts/project-collaboration-mode) without making changes. To grant visibility into a private project, add the user to that project with the **Reader** project role.

A read-only user is automatically converted to a regular user when they submit a change request, are granted access to a specific project, or create a personal access token.
