...
description: Unleash implements role-based access control at root and project levels, with predefined and custom roles for precise permission management.
keywords: rbac, role-based access control, permissions, root roles, project roles, custom roles, user groups, sso syncing
og:site_name: Unleash Documentation
og:title: Role-based Access Control in Unleash
max-toc-depth: 3
last-updated: July 30, 2026
---

<Button small outlined className="version-button" href="/support/availability">v4.0</Button>
...
users, while the Project roles are only available to [Enterprise](https://www.getunleash.io/pricing) users. The following table lists the roles, what they do, and what plans they are available in. Additionally, [Enterprise](https://www.getunleash.io/pricing) users can create their
...
| **Owner**  | Project | Users with the project owner role have full control over the project, and can add and manage other users within the project context, manage feature flags within the project, and control advanced project features like archiving and deleting the project.                                                                                                      | [Enterprise](https://www.getunleash.io/pricing) |
| **Member** | Project | Users with the project member role are allowed to view, create, and update feature flags within a project, but have limited permissions in regards to managing the project's user access and can not archive or delete the project.                                                                                                                               | [Enterprise](https://www.getunleash.io/pricing) |

## Custom root roles

<Button small outlined className="version-button" href="/support/availability">v5.4</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
Permissions to manage [API tokens](/concepts/api-tokens-and-client-keys).
...
Permissions to manage [applications](/concepts/applications).
...
<Button small outlined className="version-button" href="/support/availability">v6.9</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
Permissions to manage [context fields](/concepts/unleash-context#custom-context-fields).
...
Permissions to manage [integrations](/integrate).
...
Permissions to manage [projects](/concepts/projects).
...
<Button small outlined className="version-button" href="/support/availability">v6.8</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>

Permissions to manage global [release templates](/concepts/release-templates). To manage project-level release templates, use the Create/edit project release template [project-level permission](#project-level-permissions) instead.

| Permission Name | Description               |
|-----------------|---------------------------|
| Create release plan template      | Create global [release templates](/concepts/release-templates). |
| Update release plan template      | Update global release templates. |
| Delete release plan template      | Delete global release templates. |

#### Role permissions

Permissions to manage [roles](/concepts/rbac).
...
Permissions to manage [strategies](/concepts/activation-strategies).
...
Permissions to manage [tag types](/concepts/feature-flags#tags).
...
<Button small outlined className="version-button" href="/support/availability">v4.6</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
| Create/edit project context | Create and edit project-level [context fields](/concepts/unleash-context) within the project. |
| Create/edit project release template | Create and edit [project-level release templates](/concepts/release-templates#create-a-project-level-release-template) within the project. |
...
| Skip change requests         | Skip the change request process for environment-specific actions, such as enabling or disabling flags, in an environment where change requests are enabled. This permission does not apply to feature-level actions that affect a flag across all environments, such as archiving flags, which must go through a change request.  |

## Multiple project roles

<Button small outlined className="version-button" href="/support/availability">v5.6</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
<Button small outlined className="version-button" href="/support/availability">v6.9</Button>

The access overview page shows exactly what a user can do in Unleash, in a single view. The page is organized into two sections:

- **Instance access**: The user's root role, shown as a badge next to **Root access**. Expand the row to see every root-level permission and whether the user has it.
- **Project access**: A count of the [projects](/concepts/projects) the user can access. Use the project and environment filters to view a specific project. Each project expands to show feature and strategy permissions, and a per-environment breakdown.

To view a user's access overview, go to **Admin settings > User config > Users**, select a user, and click **Access overview**.


## User groups

<Button small outlined className="version-button" href="/support/availability">v4.14</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
<Button small outlined className="version-button" href="/support/availability">v4.18</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
[^1]: The project-level permission is still required for the

<EndpointRequestSnippet endpoint="PUT /api/admin/projects/{projectId}/features/{featureName}/variants" />
create/overwrite variants (PUT)

and

<EndpointRequestSnippet endpoint="PATCH /api/admin/projects/{projectId}/features/{featureName}/variants" />
...
<EndpointRequestSnippet endpoint="PUT /api/admin/projects/{projectId}/features/{featureName}/environments/{environment}/variants" />
create/overwrite environment variants (PUT)

and

<EndpointRequestSnippet endpoint="PATCH /api/admin/projects/{projectId}/features/{featureName}/environments/{environment}/variants" />
