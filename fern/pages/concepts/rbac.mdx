...
| **Viewer** | Root    | Users with the root viewer role can only read root resources in Unleash. Viewers can be added to specific projects as project members. Users with the viewer role may not view API tokens. Read-only access is also available as an optional add-on: a read-only user holds the Viewer root role and is automatically converted to a regular user when they submit a change request, are granted access to a project, or create a personal access token.                                                                                                                                                                          | All versions       |
...
- **Project access**: A count of the [projects](/concepts/projects) the user can access. Use the project and environment filters to drill into a specific project. Each project expands to show feature and strategy permissions, plus a per-environment breakdown.

To view a user's access overview, go to **Admin settings > User config > Users**, select a user, and click **Access overview**. Individual users can see the same view for themselves from their profile settings.
