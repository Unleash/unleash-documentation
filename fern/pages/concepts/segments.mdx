...
keywords: segments, strategy constraints, feature flags, activation strategies, context, reusable constraints
og:site_name: Unleash Documentation
og:title: Segments for Reusable Strategy Constraints
max-toc-depth: 3
last-updated: August 25, 2026
---

<Button small outlined className="version-button" href="/support/availability">v5.5</Button>
...
<div class="hero-video">
    <iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/LWMCCFcRic0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
<Frame background="subtle">
  ![Go to configure and select segments to manage your segments.](/assets/segments.png)
</Frame>
...
Before you begin, make sure that you have [permissions](/concepts/rbac) to manage segments in your project. If your project requires [change requests](/concepts/change-requests#change-requests-for-segments), deleting segments also needs to go through the same approval process.
...
The following default [resource limits](/concepts/resource-limits) apply to segments:

- A single segment can have up to 1,000 constraint values, combined across all its constraints.
- A single activation strategy can have up to 5 segments. This limit applies per strategy, not per feature flag: if a flag includes two strategies, each one can have up to five segments.
- An instance can have up to 300 segments.
...
## Manage segments with the Admin API

You can manage segments programmatically using the segments endpoints of the Admin API. For example, to create a segment:

<EndpointRequestSnippet endpoint="POST /api/admin/segments" />

To list all segments:

<EndpointRequestSnippet endpoint="GET /api/admin/segments" />

You can also [update](/api/update-segment), [delete](/api/remove-segment), and [list the strategies that use](/api/get-strategies-by-segment-id) a segment.
