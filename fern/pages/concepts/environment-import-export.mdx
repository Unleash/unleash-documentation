...
og:title: Import and export - Bootstrap Unleash instances with feature flags
description: Unleash supports import and export of feature flags and strategies at startup and during runtime. The main purpose is to bootstrap new Unleash instances with feature flags and their configuration.
keywords: import, export, state service, api, deprecated, removal
max-toc-depth: 3
---

<Button small outlined className="version-button" href="https://docs.getunleash.io/support/availability">v3.3</Button>
...
Export features (and nothing else) as YAML: `GET /api/admin/state/export?format=yaml&featureToggles=1&strategies=0&tags=0&projects=0&download=1`
...
Import data into Unleash: `POST /api/admin/state/import` with payload `{ "version": 3, "features": [{"name": "a-feature-flag", "enabled": true, "description": "#1 feature-flag"}] }`
