...
Safeguards can be configured against external metrics the same way as against internal ones, with the following differences:
...
- **No other custom-label filtering**: aside from the automatic `environment` scoping above, safeguards against external metrics do not let you filter by custom labels. The safeguard evaluates the metric across all its time series. If you need a narrower scope, expose a dedicated metric in your Prometheus or VictoriaMetrics instance with the scope already applied.
