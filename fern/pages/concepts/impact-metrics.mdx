...
### Correlate external metrics with Unleash environments

To scope an external metric to a specific Unleash environment, add an `environment` label to the metric whose value matches the Unleash environment name (for example, `environment="production"`). Safeguards in that environment will then only evaluate the matching time series.

If the `environment` label is missing, the same external metric is available for safeguards in every environment, with no per-environment filtering.
