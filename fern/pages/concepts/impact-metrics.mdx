...
Connecting an external Prometheus instance is currently in beta. Functionality may change. Share feedback in the Unleash community Slack or email beta@getunleash.io.
</Accordion>
</AccordionGroup>
...
- `GET /api/v1/label/__name__/values` — to list available metric names in the source selector.
- `GET /api/v1/labels` and `GET /api/v1/label/<name>/values` — to populate label and label-value filters in the UI.
- `GET /api/v1/query_range` — to render chart time series and to evaluate safeguard thresholds over their configured time window.

Metadata requests run when users open the chart or safeguard configuration UI. Range queries run when charts are viewed and on the regular interval at which safeguards are evaluated.
