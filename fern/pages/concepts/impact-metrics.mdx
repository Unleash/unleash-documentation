...
description: Impact metrics are lightweight, application-level time-series metrics stored and visualized directly inside Unleash. They allow you to connect specific application data to your feature flags and release plans.
keywords: impact metrics, release automation, safeguards, counters, gauges, histograms
max-toc-depth: 3
last-updated: April 15, 2026
---

<AccordionGroup>
<Accordion title="Impact metrics is an early access feature">
...
## External Impact Metrics source

<AccordionGroup>
<Accordion title="External Impact Metrics is a beta feature">
Connecting an external Prometheus or VictoriaMetrics instance is currently in beta. Functionality may change. Share feedback in the Unleash community Slack or email beta@getunleash.io.
</Accordion>
</AccordionGroup>

If you already run your own Prometheus or VictoriaMetrics instance, you can connect it to Unleash so the metrics you already collect show up alongside internal impact metrics in charts and safeguards. This is intended for customers who have existing observability infrastructure and want to reuse those metrics inside Unleash without re-instrumenting their applications through the SDK.

External metrics do not go through the Unleash ingestion flow. SDKs do not send them, Unleash does not store or scrape them, and the data stays in your own system. Unleash queries your external instance at read time when a chart or safeguard needs the data.

### Enable external metrics

- **Unleash Hosted**: contact [beta@getunleash.io](mailto:beta@getunleash.io) and we will configure the external URL for your instance.
- **Self-hosted**: set the `EXTERNAL_PROMETHEUS_IMPACT_METRICS_API` environment variable on your Unleash server to the base URL of your Prometheus or VictoriaMetrics instance. Any endpoint that speaks the Prometheus HTTP query API is supported. The environment variable is read at startup, so Unleash must be restarted for changes to take effect.

The URL is the base that Unleash appends the standard Prometheus HTTP API paths (`/api/v1/...`) to. Do not include `/api/v1` yourself. A trailing slash is accepted and ignored. Examples:

- Plain Prometheus: `https://prometheus.example.com`
- VictoriaMetrics cluster / multi-tenant: `https://victoria.example.com/select/<tenantId>/prometheus`

HTTP Basic auth is supported by embedding credentials in the URL, for example `https://user:password@prometheus.example.com`.

The URL must be reachable from the Unleash server over the network. Each request Unleash makes to your Prometheus or VictoriaMetrics instance has a 20-second timeout. Internal metrics continue to work in parallel and remain the default source.

Internal and external metrics are kept separate in Unleash. A metric with the same name can exist in both sources without conflict — each chart or safeguard is tied to the specific source it was configured with, and the metric selector lists them as distinct entries labeled with their source.

The two sources are isolated. If the external instance is unreachable or returns errors, only charts and safeguards configured against the external source are affected — internal impact metrics continue to work normally.

### Safeguards with external metrics

Safeguards can be configured against external metrics the same way as against internal ones. Two behaviors are specific to external sources:

- **Environment correlation**: to scope an external metric to a specific Unleash environment, add an `environment` label to the metric whose value matches the Unleash environment name (for example, `environment="production"`). A safeguard in the `production` environment will then only evaluate time series where `environment="production"`. If the `environment` label is missing, the same metric is available for safeguards in every environment, with no per-environment filtering.
- **Missing external configuration**: if the external URL is not configured on an Unleash server (for example, the environment variable is unset), safeguards previously configured against the external source are skipped during evaluation rather than failing the release. Reconfiguring the URL restores them.
- **No custom label filtering**: safeguards configured against external metrics do not support filtering by custom labels. The safeguard evaluates the metric across all its time series (optionally scoped by `environment` as described above). If you need a narrower scope, expose a dedicated metric in your Prometheus or VictoriaMetrics instance with the scope already applied.

### Load on your metrics backend

Because Unleash queries your external instance at read time, enabling this feature adds read load to your Prometheus or VictoriaMetrics, similar to how Grafana or other query-time tools do. Plan capacity accordingly, especially as the number of configured charts and active safeguards grows.

Unleash issues standard Prometheus HTTP API requests:

- `GET /api/v1/label/__name__/values` — to list available metric names in the metric selector.
- `GET /api/v1/labels` and `GET /api/v1/label/<name>/values` — to populate label and label-value filters in the UI.
- `GET /api/v1/query_range` — to render chart time series and to evaluate safeguard thresholds.

Metadata requests run when users open the chart or safeguard configuration UI. Range queries run when charts are viewed and each time a safeguard is evaluated.

### What's shown in the metric selector

Prometheus and VictoriaMetrics typically expose many metrics and labels that are not meaningful as application impact metrics. Unleash filters both on the way into the UI so the selector stays focused on your application-defined metrics.

**Metrics.** Excluded names include the reserved `up`, `ALERTS`, and `ALERTS_FOR_STATE`, and any metric whose name starts with a well-known infrastructure prefix such as `go_`, `process_`, `prometheus_`, `promhttp_`, `scrape_`, `vm_`, `vmagent_`, `vminsert_`, `vmselect_`, `container_`, `kube_`, or similar tooling prefixes. If one of your own metrics happens to collide with an excluded prefix, rename it to make it visible.

**Labels.** External metrics can carry custom labels, and in charts these appear as filters in the configuration UI. Safeguards do not support custom-label filtering — see [Safeguards with external metrics](#safeguards-with-external-metrics). Unleash hides a small set of infrastructure-oriented labels — Prometheus reserved labels (those starting with `__`) and common scraper/Kubernetes labels such as `instance`, `job`, `namespace`, `pod`, `container`, `node`, and `app_kubernetes_io_*`. All other labels are exposed, up to the first 10 labels per metric; any beyond that are not surfaced as filter options, to keep metadata lookups from overloading your metrics backend.
