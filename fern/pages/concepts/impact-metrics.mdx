...
- **Self-hosted**: set the `EXTERNAL_PROMETHEUS_IMPACT_METRICS_API` environment variable on your Unleash server to the base URL of your Prometheus or VictoriaMetrics instance. Any endpoint that speaks the Prometheus HTTP query API is supported.

The URL must be reachable from the Unleash server over the network. Internal metrics continue to work in parallel and remain the default source.

Internal and external metrics are kept in separate namespaces, identified by their source. A metric with the same name can exist in both sources without conflict — each chart or safeguard is tied to the specific source it was configured with, and the metric selector shows them as distinct entries.
...
- **Environment correlation**: to scope an external metric to a specific Unleash environment, add an `environment` label to the metric whose value matches the Unleash environment name (for example, `environment="production"`). Safeguards in that environment will then only evaluate the matching time series. If the `environment` label is missing, the same metric is available for safeguards in every environment, with no per-environment filtering.
...
Prometheus and VictoriaMetrics typically expose many infrastructure and runtime metrics (Go runtime stats, scrape health, Kubernetes, and similar) that are not meaningful as application impact metrics. Unleash applies heuristics to hide these from the metric selector so you only see your application-defined metrics.

### Custom labels

External metrics can carry custom labels you define in Prometheus or VictoriaMetrics, and they are available as filters when you configure charts. Safeguards against external metrics do not support custom-label filtering (see [Safeguards with external metrics](#safeguards-with-external-metrics)). Unleash hides a small set of infrastructure-oriented labels that are rarely useful for feature analysis — Prometheus reserved labels (those starting with `__`) and common scraper/Kubernetes labels such as `instance`, `job`, `namespace`, `pod`, `container`, `node`, and `app_kubernetes_io_*`. All other labels are exposed.

To keep metadata lookups bounded and avoid overloading your metrics backend when listing label values, Unleash surfaces at most 10 labels per metric in the UI. If a metric has more, the remaining labels are not shown as filter options.
