...
External metrics can carry custom labels you define in Prometheus or VictoriaMetrics, and they are available as filters when you configure charts and safeguards. Unleash hides a small set of infrastructure-oriented labels that are rarely useful for feature analysis — Prometheus reserved labels (those starting with `__`) and common scraper/Kubernetes labels such as `instance`, `job`, `namespace`, `pod`, `container`, `node`, and `app_kubernetes_io_*`. All other labels are exposed.
