...
- **[Release management](#automate-release-progression)**: Automate milestone progression or trigger [safeguards](#configure-safeguards) to pause a rollout.

By combining impact metrics with [release templates](/concepts/release-templates), you can automate milestone progression and pause rollouts when metrics exceed defined thresholds. For example:
...
<Tabs>
<Tab title="Unleash Hosted">

Contact [beta@getunleash.io](mailto:beta@getunleash.io) to configure the external URL for your instance.

</Tab>
<Tab title="Self-hosted">

Set the `EXTERNAL_PROMETHEUS_IMPACT_METRICS_API` environment variable on your Unleash server to the base URL of your Prometheus or VictoriaMetrics instance. Any endpoint that speaks the Prometheus HTTP query API is supported. Unleash must be restarted for changes to take effect.
...
HTTP Basic Auth is supported by embedding credentials in the URL (for example, `https://user:password@prometheus.example.com`). Credentials in URLs can appear in server logs and environment variable listings. Use a secrets manager or other secure mechanism to manage the environment variable value.
...
Safeguards automatically pause a rollout when metrics indicate a problem. You can configure safeguards against both [internal](#internal-metrics) and [external](#external-metrics) metrics.

First, verify your metric is available by selecting **Add safeguard** and checking the dropdown. For [internal metrics](#internal-metrics), it may take up to 60 seconds for a new metric to appear because the SDK sends metrics on a default 60-second interval.

To add a safeguard:

1. Select **Add safeguard** on your release plan.
2. Select your impact metric from the dropdown. The selector labels each metric with its source.
...
When the metric exceeds the threshold, Unleash pauses the automated progression.
