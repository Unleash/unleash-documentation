...
Unleash combines the selected context field with the strategy's `groupId` before hashing. The `groupId` acts as a grouping input: it gives each flag its own independent distribution, so the same user can fall inside the rollout for one flag and outside it for another.
