...
og:title: Activation strategies - Control feature flag targeting and rollouts
description: Activation strategies determine who should get a feature. They allow you to enable and disable features for certain users without having to redeploy your application.
keywords: activation strategies, gradual rollout, constraints, targeting, feature flags
max-toc-depth: 3
...
![A feature flag with two strategies](/assets/activation-strategies-example.png)
</Frame>
...
When an activation strategy has multiple constraints, then every constraint must be evaluated to true for the strategy to be evaluated. This behavior is equivalent to the AND logical operator.

The number of constraints per strategy and the number of values per constraint are limited by your plan; see [Resource limits](/concepts/resource-limits) for the exact limits. If you need to target a large audience, avoid long value lists and use higher-level attributes instead; see [Managing constraints](/guides/managing-constraints) for practical guidance.
...
<div class="hero-video">
    <iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/kqtqMFhLRBE?si=Ut03Q6TspYC2N3Jr" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
| `remoteAddress` | `IN_CIDR`  | `10.0.0.0/8, 192.168.1.10`     | Evaluates to `true` if the request comes from an IP address in the `10.0.0.0/8` range or from `192.168.1.10`.                                      |

### Constraint operators

Constraint operators help you define the conditional statements that get evaluated as part of the constraint. [Basic operators](#basic-operators) are available in all versions and SDKs. Numeric, date and time, string, and SemVer operators require Unleash version 4.9+. All advanced operators, including the [regex](#regex-operator) and [IP address](#ip-address-operator) operators, require [SDK compatibility](/sdks#strategy-constraints).
...
| `SEMVER_GTE` | The context field is greater than or equal to the provided value. |
| `SEMVER_LT` | The context field is strictly less than the provided value.    |
| `SEMVER_LTE` | The context field is less than or equal to the provided value. |

#### Regex operator

The regex operator checks the context field against a regular expression. It only accepts a single value.

| Operator | Description |
|----------|-------------|
| `REGEX`  | The context field matches the provided regular expression. |

The operator is case-sensitive by default, and you can set it to be case-insensitive. The expression can match any part of the context field; use `^` and `$` to match the whole value. For example, `^user\d{3}$` matches `user123` but not `user12`.

The operator doesn't support lookaheads or backreferences. If the context field is missing, the constraint evaluates to false.

The regex operator requires Unleash Enterprise Edge 20.1.9 or later. Frontend SDKs support it when connected to Unleash or Unleash Enterprise Edge. For backend SDK support, see [SDK compatibility](/sdks#strategy-constraints).

#### IP address operator

The IP address operator checks whether the context field is an IP address within the provided values. Values can be single IP addresses, such as `192.168.1.10`, or IP ranges in [CIDR notation](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing#CIDR_notation), such as `192.168.1.0/24`. Both IPv4 and IPv6 are supported. The operator accepts multiple values.

| Operator  | Description |
|-----------|-------------|
| `IN_CIDR` | The context field is an IP address that matches any of the provided IP addresses or ranges. |

The context field is typically `remoteAddress`. If the context field is missing, the constraint evaluates to false.

The IP address operator requires Unleash Enterprise Edge 20.2.0 or later. Frontend SDKs support it when connected to Unleash or Unleash Enterprise Edge. For backend SDK support, see [SDK compatibility](/sdks#strategy-constraints).
...
<Frame background="subtle">
![A feature flag with two strategies](/assets/add-strategy-modal.png)
</Frame>
...
<Frame background="subtle">
![A strategy with five parameters, one of each type.](/assets/strategy-parameters-ui-controls.png)
</Frame>
