...
description: The Unleash context contains information related to the current feature flag request and is used to evaluate activation strategies and calculate stickiness.
keywords: unleash context, context fields, custom context fields, static fields, dynamic fields, properties, legal values, custom stickiness
og:site_name: Unleash Documentation
og:title: Unleash Context for Feature Flag Evaluation
max-toc-depth: 3
...
You can add context fields at the global or project level. Global context fields are available across all projects. Project-level context fields are only available in [strategy configuration](/concepts/activation-strategies) and the [playground](/concepts/playground) for users with access to that project, and are not included in the global context fields overview.

<Tabs>
<Tab title="Global context field">

To add a global context field in the Admin UI, do the following:
...
<Tab title="Project-level context field">

<Button small outlined className="version-button" href="/support/availability">v7.6</Button>

To add a project-level context field in the Admin UI, do the following:

1. Open the project and go to **Settings** > **Context fields**, then click **New context field**.
2. Enter a context name.
3. In **Legal value**, enter a value you want the field to support and click **Add**. You can add additional values if needed.
4. Optionally, enable [custom stickiness](/concepts/stickiness#custom-stickiness) if you'd like to use this field to group users for a gradual rollout strategy.
5. Click **Create context**.
...
### Clone a context field

<Button small outlined className="version-button" href="/support/availability">v8.1</Button>

You can clone an existing context field instead of retyping its definition. This is useful when you want to recover from a typo in a context field name, since names cannot be changed after creation, or when you want to create a similar field without re-entering a long list of legal values.

To clone a context field, open the context field list, either globally under **Configure** > **Context fields** or in a project under **Settings** > **Context fields**, and select **Clone** on the field you want to copy. This opens the create form pre-filled with the source field's description, legal values, and stickiness setting. The name is pre-filled as `<source-name>_clone`, so you can correct it before saving.
...
<Frame background="subtle">
  ![A strategy constraint form with a constraint set to "region". The "values" input is a dropdown menu containing the options "Africa", "Asia", "Europe", and "North America", as defined in the preceding paragraph.](/assets/constraints_legal_values.png)
</Frame>

### Custom stickiness

<Note title="SDK compatibility">
