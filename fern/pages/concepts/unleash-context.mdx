...
You can add context fields at the global or project level. Global context fields are available across all projects. Project-level context fields <Button small outlined className="version-button" href="/support/availability">v7.6</Button> are only available in [strategy configuration](/concepts/activation-strategies) and the [playground](/concepts/playground) for users with access to that project, and are not included in the global context fields overview.
