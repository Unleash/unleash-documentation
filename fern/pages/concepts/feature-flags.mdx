...
og:title: Feature flags - Release, test, and manage features without changing code
description: Feature flags are a core concept of Unleash. They allow you to release, test, and manage features and functionality across your application without changing the source code.
keywords: feature flags, activation strategies, environments, lifecycle, dependencies
max-toc-depth: 3
...
<Button small outlined className="version-button" href="/support/availability">v3.5</Button>
...
![Marking a feature flag as stale](/assets/mark-flag-stale.png)
</Frame>
...
<Button small outlined className="version-button" href="/support/availability">v6.2</Button>
...
<Frame background="subtle">
![Feature lifecycle](/assets/feature-lifecycle.png)
</Frame>
...
The **Status** column in **Flags overview** summarizes each flag's rollout state based on its [lifecycle stage](#lifecycle-stages) and current environment configuration. It helps you spot flags that need attention without opening each one.

The status message depends on the lifecycle stage:

- In the **Define** stage, the status indicates what's preventing the flag from receiving metrics. For example, non-production environments may have no [activation strategies](/concepts/activation-strategies), no enabled strategies, or no traffic to an enabled environment.
- In the **Develop** stage, the status shows whether any environment is enabled.
- In the **Production** and **Cleanup** stages, the status reflects the production rollout: how many production environments the flag is enabled in, whether production environments or strategies are configured, and the current milestone for flags using [release templates](/concepts/release-templates).

Common values include **No traffic**, **No strategies**, **No enabled strategies**, **Paused**, **In X out of Y production environments**, **No production environments**, and **Milestone: \<milestone-name\>**. If no condition applies, the column is empty.

## Feature flag dependencies

<Button small outlined className="version-button" href="/support/availability">v5.7</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>

<Info> This feature also requires a minimum Unleash Edge version of `13.1+` and [SDK compatibility](/sdks#feature-compatibility-in-backend-sdks) for variants. </Info>
...
<Frame background="subtle">
![Feature parent flag](/assets/add-parent-flag.png)
</Frame>
...
<Button small outlined className="version-button" href="/support/availability">v5.12</Button> <Button small href="https://www.getunleash.io/plans" className="cta-button">Enterprise</Button>
...
<Frame background="subtle">
![Archive a feature flag](/assets/archive-feature-flag.png)
</Frame>
...
<Frame background="subtle">
![Archive a feature flag](/assets/revive-archived-flag.png)
</Frame>
...
<Button small outlined className="version-button" href="/support/availability">v3.11</Button>
...
<Button small outlined className="version-button" href="/support/availability">v7.0</Button>
...
<Button small outlined className="version-button" href="/support/availability">v7.2</Button>
