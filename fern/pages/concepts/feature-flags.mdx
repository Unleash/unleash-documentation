...
The status message depends on the lifecycle stage. In the **Define** stage, the status shows why the flag hasn't received metrics yet. For example, **No strategies** or **No enabled strategies** indicate missing [activation strategies](/concepts/activation-strategies) or have strategies that aren't enabled.
