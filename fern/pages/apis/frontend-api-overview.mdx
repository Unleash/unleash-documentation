...
The Frontend API is designed for [frontend SDKs](/sdks/react) running in browsers, mobile apps, and other client-side environments. Unlike the [Client API](/api/client-api), it only returns enabled feature flags for a specific [Unleash Context](/concepts/unleash-context), providing security and performance benefits.
...
| Aspect | [Client API](/api/client-api) | Frontend API |
|--------|------------|--------------|
...
| **Evaluation** | Server-side by SDK. | Evaluated by the Unleash server or [Unleash Enterprise Edge](/unleash-edge/overview). |
...
For production applications, use [Unleash Edge](/unleash-edge/overview) to improve performance, privacy, and reduce latency. Edge caches evaluations closer to your users and provides built-in fallback mechanisms.
