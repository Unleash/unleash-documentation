...
By default, Unleash calculates stickiness from the `userId` context field and falls back to the `sessionId` if the `userId` is unavailable. The strategy's `groupId` is also part of the calculation, so each flag distributes users independently.
...
You can also use [impact metrics](/concepts/impact-metrics) to correlate flag exposure with product and system metrics directly in Unleash, without sending impression events to an external analytics tool.
