...
description: Learn how to implement A/B testing using feature flags with Unleash. Run multivariate tests, target users, track metrics, and roll out winning variants.
keywords: a/b testing, feature flags, experimentation, variants, unleash
og:site_name: "Unleash Documentation"
og:title: "Implement A/B testing using feature flags"
max-toc-depth: 3
...
<Frame background="subtle">
  ![Create a feature flag by filling out the form fields.](/assets/use-case-create-experiment-flag.png)
</Frame>
...
<Frame background="subtle">
  ![Anatomy of an activation strategy](/assets/anatomy-of-unleash-strategy.png)
</Frame>
...
<Frame background="subtle">
  ![Add your first strategy from the flag view in Unleash.](/assets/add-strategy.png)
</Frame>
...
<Frame background="subtle">
  ![In the gradual rollout form, you can configure the parameters of your A/B tests and releases.](/assets/use-case-experiment-gradual-rollout.png)
</Frame>
...
<Frame background="subtle">
  ![This diagram breaks down how strategy variants sit on top activation strategies for flags in Unleash.](/assets/tutorial-building-blocks-strategy-variants.png)
</Frame>
...
<Frame background="subtle">
  ![You can configure multiple strategy variants for A/B testing within the gradual rollout form.](/assets/use-case-experiment-variants.png)
</Frame>
...
For example, user `uuid1234` should be the target of `variantA` regardless of their session. The original subset of users that get `variantA` will continue to experience that variation of the feature over time. At Unleash, we call this [stickiness](/concepts/stickiness). You can define the parameter of stickiness in the gradual rollout form.

By default, Unleash calculates stickiness from the `userId` context field and falls back to the `sessionId` if the `userId` is unavailable. The strategy's `groupId` is also part of the calculation. It defaults to the feature flag name, giving each flag an independent distribution; assigning the same `groupId` to multiple flags correlates their rollouts.
...
<Frame background="subtle">
  ![Enable impression data in the strategy rollout form for your flag.](/assets/use-case-experiment-enable-impression-data.png)
</Frame>
...
You can also use [impact metrics](/concepts/impact-metrics) to monitor aggregate product and system metrics during a rollout and automate [safeguards](/concepts/impact-metrics#configure-safeguards). To attribute outcomes to individual flag or variant exposure in an A/B test, send impression events to an analytics tool.
...
<Frame background="subtle">
  ![This is a graph comparing traditional A/B testing and multi-arm bandit selection.](/assets/use-case-ab-testing-vs-bandit.png)
</Frame>
