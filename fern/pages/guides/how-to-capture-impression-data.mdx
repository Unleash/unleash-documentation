...
description: Capture Unleash impression data from feature flags and send events to analytics tools like PostHog or Google Analytics.
keywords: send, impression, data, feature flags, unleash
og:site_name: "Unleash Documentation"
og:title: "Send impression data to analytics tools"
max-toc-depth: 3
...
<iframe src="https://www.youtube-nocookie.com/embed/bxYdeMb9ffw" width="100%" height="450px" frameBorder="0" allowFullScreen></iframe>
...
<Frame background="subtle">
  ![The create feature flag form. There's a flag at the end of the form that enables or disables impression data. It's labeled "impression data".](/assets/enable-impression-data.png)
</Frame>

To create a feature flag with impression data enabled, set the `impressionData` option to `true` in the request payload, as seen below. For more options, check the reference docs on creating features:
...
<Frame background="subtle">
  ![The create feature flag form. There's a flag at the end of the form that enables or disables impression data. It's labeled "impression data".](/assets/enable-impression-data-existing-toggle.png)
</Frame>

To enable impression data for an existing flag, use the API's flag patching functionality:
