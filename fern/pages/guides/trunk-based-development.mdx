...
description: "Implement trunk-based development with Unleash feature flags: merge faster, keep trunk deployable, control rollouts, and roll back safely."
keywords: implement, trunk, based, feature flags, unleash
og:site_name: "Unleash Documentation"
og:title: "Implement trunk-based development using feature flags"
max-toc-depth: 3
last-updated: August 10, 2026
---

In trunk-based development, you merge code into the main branch, the trunk, as quickly as possible. This reduces the complexity of long-lived feature branches, minimizes merge conflicts, and lets teams continuously integrate and test their code. It also means unfinished features get deployed to production, which is where feature flags come in: they keep unfinished code hidden until it is ready.

In this tutorial, you'll use Unleash feature flags to implement trunk-based development safely.
...
Keep in mind that a feature is often split into pieces that span many short-lived branches. A feature flag prevents an incomplete feature from being exposed before it's done.
...
> At that point, you're more likely to break something and it’s harder for developers to review them properly.
...
Unleash supports all of these trunk-based development styles; choose the one that fits your team.
...
<Frame background="subtle">
  ![Create a new feature flag in the Unleash Admin UI.](/assets/use-case-new-flag.png)
</Frame>
...
<Frame background="subtle">
  ![Create a feature flag by filling out the form fields.](/assets/use-case-create-tbd-flag.png)
</Frame>
...
<Frame background="subtle">
  ![Create a tag with relevant data to better organize your feature flags. A modal will pop up to name and save as many tags as you need.](/assets/use-case-tbd-tagging.png)
</Frame>
...
<Frame background="subtle">
  ![In your flag settings form, there is an optional description field for adding more details about the flag you're creating.](/assets/use-case-tbd-edit-flag.png)
</Frame>
...
<Frame background="subtle">
  ![Add a strategy to configure a release process for your flag.](/assets/use-case-tbd-add-strategy.png)
</Frame>
...
Once a feature is fully rolled out and stable, remove the feature flag from your code and archive it in Unleash. This reduces technical debt in the codebase and prevents unnecessary complexity.
...
3. Archive the flag in Unleash; see [Archive a feature flag](/concepts/feature-flags#archive-a-feature-flag). Archiving keeps the flag's history, and you can revive it later if needed.

Do the code cleanup before archiving: if you archive a flag that is still referenced in code, `isEnabled` calls for it return the fallback value, which can silently switch users back to the old code path.

To find cleanup candidates, use the [feature flag lifecycle](/concepts/feature-flags#feature-flag-lifecycle): it highlights flags that have been fully rolled out and are ready to be completed and archived. Regularly review which flags are active, which environments they are enabled in, and when they were last modified, and plan flag cleanup as part of your normal development workflow, for example by creating a cleanup task whenever a release flag reaches 100% rollout.
