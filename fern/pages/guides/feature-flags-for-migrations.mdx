...
The cloud providers describe the interception layer abstractly. A feature flag is the concrete routing decision inside it. Instead of hardcoding which routes have migrated, the interception layer evaluates a flag per request:
...
<Frame background="subtle">
  ![A progressive exposure ladder from internal users and a beta segment through a gradual rollout to 100%, with safeguards pausing the release if metrics cross a threshold](/assets/migration-progressive-exposure.svg)
</Frame>
...
Unleash's gradual rollout strategy computes a hash of a [stickiness](/concepts/stickiness) field, such as user ID or session ID, so the same user always lands on the same side of the flag at a given rollout percentage, across requests, sessions, and services. When you increase the percentage, users only ever move from legacy to new, never back and forth. Pass the same [Unleash context](/concepts/unleash-context) fields at every evaluation point so the hash is computed on the same input everywhere.
