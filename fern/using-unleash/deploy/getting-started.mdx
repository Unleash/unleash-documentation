...
description: "Unleash offers several , including fully self-hosted setups. This guide helps you set up Unleash Open Source or Unleash Enterprise in your own environment using Docker."
---
...
## Start Unleash server[​](#start-unleash-server "Direct link to Start Unleash server")
...
```
git clone git@github.com:Unleash/unleash.gitcd unleashdocker compose -f docker-compose-enterprise.yml up -d
```
...
#### Create Docker network[​](#create-docker-network "Direct link to Create Docker network")
...
#### Start PostgreSQL database container[​](#start-postgresql-database-container "Direct link to Start PostgreSQL database container")
...
```
docker run -d \  -e POSTGRES_PASSWORD=your_secure_password \  -e POSTGRES_USER=unleash_user \  -e POSTGRES_DB=unleash \  --network unleash \  --name postgres \  postgres:17 # or any 14+ version
```

#### Start Unleash server container[​](#start-unleash-server-container "Direct link to Start Unleash server container")
...
```
docker run -d -p 4242:4242 \  -e DATABASE_HOST=postgres \  -e DATABASE_NAME=unleash \  -e DATABASE_USERNAME=unleash_user \  -e DATABASE_PASSWORD=your_secure_password \  -e DATABASE_SSL=false \  --network unleash \  --name unleash \  --pull=always \  unleashorg/unleash-enterprise
```

```
docker run -d -p 4242:4242 \  -e DATABASE_HOST=postgres \  -e DATABASE_NAME=unleash \  -e DATABASE_USERNAME=unleash_user \  -e DATABASE_PASSWORD=your_secure_password \  -e DATABASE_SSL=false \  --network unleash \  --name unleash \  --pull=always \  unleashorg/unleash-server
```

## Log in to the Unleash Admin UI[​](#log-in-to-the-unleash-admin-ui "Direct link to Log in to the Unleash Admin UI")
...
![Unleash Admin UI log in screen](../../images/using-unleash/deploy/assets/images/quickstart-login-bbd9175782aa4a754337df8478a476cf.png)
...
### Install your license[​](#install-your-license "Direct link to Install your license")
...
## Test your server connection[​](#test-your-server-connection "Direct link to Test your server connection")
...
```
curl --location --request POST 'http://localhost:4242/api/admin/features' \--header 'Authorization: <API_TOKEN>' \--header 'Content-Type: application/json' \--data-raw '{  "name": "Test Feature Flag",  "description": "Feature flag for testing",  "type": "release",  "enabled": false,  "stale": false,  "strategies": [    {      "name": "default",      "parameters": {}    }  ]}'
```

## Disable version check[​](#disable-version-check "Direct link to Disable version check")

By default, your self-hosted Unleash instance periodically checks [https://version.unleash.run](https://version.unleash.run) to inform you about new releases. This check sends a unique, anonymous instance ID.
...
[Edit this page](https://github.com/Unleash/unleash/edit/main/website/docs/using-unleash/deploy/getting-started.md)
