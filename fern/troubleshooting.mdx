...
description: "This guide helps you troubleshoot various situations you might encounter when working with Unleash feature flags, including flags not being returned, users not being exposed as expected, unexpected A/B test results, and CORS errors."
---

## My feature flag is not returned or my users are not exposed[​](#my-feature-flag-is-not-returned-or-my-users-are-not-exposed "Direct link to My feature flag is not returned or my users are not exposed")
...
#### Verify feature configuration[​](#verify-feature-configuration "Direct link to Verify feature configuration")
...
#### SDK `ready` event[​](#sdk-ready-event "Direct link to sdk-ready-event")
...
### Token configuration[​](#token-configuration "Direct link to Token configuration")

#### Check token type[​](#check-token-type "Direct link to Check token type")
...
#### Check token access[​](#check-token-access "Direct link to Check token access")
...
### Context and stickiness[​](#context-and-stickiness "Direct link to Context and stickiness")

#### Gradual rollout strategy and stickiness[​](#gradual-rollout-strategy-and-stickiness "Direct link to Gradual rollout strategy and stickiness")
...
### Using the Unleash playground[​](#using-the-unleash-playground "Direct link to Using the Unleash playground")
...
### Alternative: Using variants for disabled/enabled states[​](#alternative-using-variants-for-disabledenabled-states "Direct link to Alternative: Using variants for disabled/enabled states")
...
* Next, configure [strategy variants](/reference/strategy-variants) to represent "enabled" and "disabled" states. You can assign percentages to these variants (e.g., 50% "enabled", 50% "disabled").
  ...
  ![Using enabled and disabled variants](images/assets/images/enabled-disabled-variants-c0b417c52f4532365112d0eede39bf79.png) *This flag itself is enabled in development and adds a 50%/50% split between disabled/enabled variants. This is essentially the same as a gradual rollout of 50% but using variants.*
...
* This approach can also be combined with more complex constraint-based targeting.
  ...
  ![Using enabled and disabled variants with constraints](images/assets/images/enabled-disabled-variants-complex-912468343e0699b1d9827fafaadb044a.png) *This flag returns an "enabled" variant for clients with a specific `semver` and performs a percentage split for the remaining clients.*

## My A/B tests are producing unexpected results[​](#my-ab-tests-are-producing-unexpected-results "Direct link to My A/B tests are producing unexpected results")

If your A/B tests or experiments are producing unexpected results:

#### Prerequisite check[​](#prerequisite-check "Direct link to Prerequisite check")
...
#### Verify gradual rollout percentage[​](#verify-gradual-rollout-percentage "Direct link to Verify gradual rollout percentage")
...
#### Check stickiness and context[​](#check-stickiness-and-context "Direct link to Check stickiness and context")
...
#### Ensure variants are correctly configured[​](#ensure-variants-are-correctly-configured "Direct link to Ensure variants are correctly configured")
...
* For a simple 50-50 A/B test, your variants should be configured accordingly (e.g., two variants, "A" and "B", with appropriate weighting or rollout distribution if not handled by a parent strategy).
  ...
  ![An example of variants configured for an A/B test](images/assets/images/troubleshooting-flag-abn-test-unexpected-result-variants-83080779e23552141827c3757ef440e1.png)

#### Double-check SDK code for variant handling[​](#double-check-sdk-code-for-variant-handling "Direct link to Double-check SDK code for variant handling")
...
* For example, using the [Unleash React SDK](/reference/sdks/react), you might follow the [check variants](/reference/sdks/react#check-variants) section. Given the example variants "A" and "B", your code might look like this:
  ...
  ```
  import { useVariant } from '@unleash/proxy-client-react';export const TestComponent = () => {  const variant = useVariant('ab-test-flag'); // 'ab-test-flag' is your feature flag name  if (variant.name === 'A') {    return <AComponent />;  } else if (variant.name === 'B') {    return <BComponent />;  }  // Fallback or default component if the flag is off or variant is not recognized  return <DefaultComponent />;};
  ```

#### Use the Unleash playground[​](#use-the-unleash-playground "Direct link to Use the Unleash playground")
...
## My requests are blocked due to CORS issues[​](#my-requests-are-blocked-due-to-cors-issues "Direct link to My requests are blocked due to CORS issues")
...
### Configuring CORS in the Unleash admin UI (for Unleash server)[​](#configuring-cors-in-the-unleash-admin-ui-for-unleash-server "Direct link to Configuring CORS in the Unleash admin UI (for Unleash server)")
...
### Configuring CORS for Unleash Edge[​](#configuring-cors-for-unleash-edge "Direct link to Configuring CORS for Unleash Edge")
...
### Verifying the `Access-Control-Allow-Origin` header[​](#verifying-the-access-control-allow-origin-header "Direct link to verifying-the-access-control-allow-origin-header")
...
## I don't see an Unleash feature in the Admin UI[​](#i-dont-see-an-unleash-feature-in-the-admin-ui "Direct link to I don't see an Unleash feature in the Admin UI")
...
## Got locked out of an Admin account after configuring SCIM[​](#got-locked-out-of-an-admin-account-after-configuring-scim "Direct link to Got locked out of an Admin account after configuring SCIM")

When you integrate Unleash with an identity provider (IdP) like [Okta](/how-to/how-to-setup-provisioning-with-okta) or [Entra ID](/how-to/how-to-setup-provisioning-with-entra) via [SCIM](/reference/scim), your IdP becomes the source of truth for user information. This can have an unintended side effect: users, including the one who set up the integration, might lose their admin permissions.
...
#### Manage roles with user groups[​](#manage-roles-with-user-groups "Direct link to Manage roles with user groups")
...
#### Create a recovery admin account[​](#create-a-recovery-admin-account "Direct link to Create a recovery admin account")
...
### What if I'm already locked out?[​](#what-if-im-already-locked-out "Direct link to What if I'm already locked out?")
...
## My Network Tab is empty, and I don't see any data.[​](#my-network-tab-is-empty-and-i-dont-see-any-data "Direct link to My Network Tab is empty, and I don't see any data.")
...
[Edit this page](https://github.com/Unleash/unleash/edit/main/website/docs/troubleshooting.mdx)
